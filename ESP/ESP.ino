...
class SolenoidController {
  private:
    ...
    virtual bool* getNotes(boardtime currentTime){
      ...
      copy(begin(notes[currentRound]),end(notes[currentRound]),begin(currentNote));
      if(timeInterval <= diffTime && currentRound != 0){
        bool lastNote[keys];
        bool finalNote[keys];
        copy(begin(notes[currentRound - 1]),end(notes[currentRound - 1]),begin(lastNote));
        
        for(int i=0;i<keys;i++){
          if(lastNote[i] == true && currentNote[i] == false){
            finalNote[i] = true;
          } else {
            finalNote[i] = currentNote[i];
          }
        }
        return finalNote;
      } else {
  ...
  public:
    ...
    virtual void periodUpdate(){
      ...
      if(this->isPlaying){ //playing
        bool* note = this->getNotes(currentTime);
...
SolenoidController *mySolenoid;

void setup() {
  mySolenoid = new SolenoidController();
  ESP.wdtDisable();
  Serial.begin(115200);
  mySolenoid->startPlaying();
}

void loop() {
  mySolenoid->periodUpdate();
  ESP.wdtFeed();
  delay(10);
}
